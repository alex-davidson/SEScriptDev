    ...
    internal static class DebugWriterExtensions
    {
        public static void Write(this IDebugWriter writer, string phase, ref Tableau tableau)
        {
            ...
            for (var c = Tableau.FirstConstraintRow; c < tableau.RowCount; c++)
            {
                ...
                sb.Append(" = ");
                if (SimplexOp.TryGetBasicSolution(ref tableau, basic, out var value))
                {
                    sb.Append(value);
                }
                else
        ...
        public static void WritePivot(this IDebugWriter writer, ref Tableau tableau, int row, int column)
        {
            if (writer == null) return;
